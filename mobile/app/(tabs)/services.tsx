  ...
  const fetchData = async (skip: number = 0) => {
    try {
      const response = await axios.get(
        `${process.env.EXPO_PUBLIC_BACKEND_URI}/service-entry?skip=${skip}`
      );
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "#fff" }}>
      <View style={{ padding: 10 }}>
        <Link href="/service/" style={styles.addButton}>
          <Text
            ...
            <TouchableOpacity onPress={() => navigateToDetails(entry._id)}>
              <View style={styles.listItem}>
                ...
                <View style={styles.rightDetails}>
                  ...
                  <Text style={styles.listItemText}>{entry.quotation}</Text>
                </View>
              </View>
            </TouchableOpacity>
          </View>
